...
export default async function handler(
  ...
  if (fs.existsSync('./public/config/tags.json')) res.status(405).send({message: "❌ - Failed to create tags.json"});
  if (fs.existsSync('./public/config/extensions.json')) res.status(405).send({message: "❌ - Failed to create extensions.json"});

  fs.writeFile("./public/config/users.json", usersJson, (err: any) => { res.status(405).send({message: "❌ - Failed to create users.json", data: err})});
  fs.writeFile("./public/config/files.json", filesJson, (err: any) => { res.status(405).send({message: "❌ - Failed to create files.json", data: err})});
  fs.writeFile("./public/config/servers.json", serversJson, (err: any) => { res.status(405).send({message: "❌ - Failed to create servers.json", data: err})});
  fs.writeFile('./public/config/tags.json', tagsJson, (err: any) => { res.status(405).send({message: "❌ - Failed to create tags.json", data: err})});
  fs.writeFile('./public/config/extensions.json', extensionJson, (err: any) => { res.status(405).send({message: "❌ - Failed to create extensions.json", data: err})});
